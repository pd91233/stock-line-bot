...
def execute_force_refresh():
    ...
    try:
        ...
        try:
            ...
            soup = BeautifulSoup(class_res.text, 'html.parser')
            # 簡化族群清單以確保解析成功率
            target_industries = ["半導體", "電腦週邊", "電子零組件", "通信網路", "生技醫療", "金融保險", "鋼鐵工業", "航運業", "建材營造", "電機機械"]
            leaderboard = {}
            for ind in target_industries:
                ind_element = soup.find(text=re.compile(ind))
                if ind_element:
        ...
        if res_json.status_code == 200:
            ...
            if isinstance(raw_data, list):
                # 這裡補上簡單的篩選邏輯，確保 ai_payload 有內容
                ai_payload = [{"name": item.get("商品"), "code": item.get("代碼"), "z": 0.0, "chg": 0.0} for item in raw_data[:5]]
            ...
            update_cache({
                ...
                "stocksText": " ｜ ".join([f"{s['name']}({s['code']})" for s in ai_payload])
            })
            print("✅ [戰術回報] 快取寫入成功！")
